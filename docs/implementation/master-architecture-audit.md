...
| 17 | **Candle cannot bypass CPU budget** | `CpuIsolationPlan` (`crates/attic-semantic/src/cpu_isolation.rs`) divides granted CPU threads across lanes, establishes startup thread ceilings (`configure_startup_thread_ceiling`), and enforces dynamic Qwen CPU containment through `ResourceOrchestrator` admission control, batch budgeting, and dedicated lane pools without post-startup environment variable manipulation. | **PASS** |
