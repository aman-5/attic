...
-- Description: Create the complete initial schema for Attic workspace intelligence.
-- Applies to: workspace/.mcp/index.db
-- Schema version: 1
--
-- All DDL statements use CREATE TABLE IF NOT EXISTS / CREATE INDEX IF NOT EXISTS
-- so that this migration is idempotent and safe to re-run after a crash.
--
-- Table namespace convention:
--   core_* : canonical intelligence / source state (durable; never wiped on restart)
--   ops_*  : operational / task / scheduling state (can be rebuilt after crash)
--   fts_*  : full-text search virtual tables (derived; rebuildable)
--
-- IMPORTANT: Do NOT add any application logic here. This file is pure DDL.
-- IMPORTANT: Secret bytes MUST NEVER appear in retrieval_text, FTS content,
--            or any text column in this schema (contract: secrets.md).
...
-- ============================================================
-- SECTION 2: Core Repository and Revision Tables
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_index_generations_revision
    ON core_index_generations(source_revision_id, created_at DESC);

-- ============================================================
-- SECTION 4: File Identity and Occurrence
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_symbol_occ_definition
    ON core_symbol_occurrences(symbol_identity_id)
    WHERE is_definition = 1;

-- ============================================================
-- SECTION 6: Structural Nodes (AST segments)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_structural_nodes_stale
    ON core_structural_nodes(freshness_state)
    WHERE freshness_state IN ('STALE', 'INVALID');

-- ============================================================
-- SECTION 7: Retrieval Units
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_retrieval_units_semantic_pending
...
-- ============================================================
-- SECTION 8: Relationships (Cross-file and Cross-repo)
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_knowledge_items_file
    ON core_knowledge_items(file_occurrence_id);
CREATE INDEX IF NOT EXISTS idx_knowledge_items_repo
...
CREATE INDEX IF NOT EXISTS idx_evidence_freshness
    ON core_evidence(freshness_state)
    WHERE freshness_state IN ('STALE', 'INVALID');

-- ============================================================
-- SECTION 11: Invalidation Records (DAG audit trail)
-- ============================================================
...
-- ============================================================
-- SECTION 12: Full-Text Search Virtual Tables
-- ============================================================

-- FTS over retrieval unit content (external content table; authoritative in core_retrieval_units)
-- INVARIANT: retrieval_text MUST NOT contain secret bytes.
...
-- FTS over symbol qualified names for fuzzy symbol lookup
...
-- ============================================================
-- SECTION 13: Operational Tables
-- ============================================================

-- ops_tasks: general task queue for background indexing and maintenance work
...
-- ops_indexing_log: tracks each indexing run (FULL_REINDEX or INCREMENTAL_UPDATE)
-- Used by crash recovery (Step R-4) to detect incomplete runs.
CREATE TABLE IF NOT EXISTS ops_indexing_log (
...
CREATE INDEX IF NOT EXISTS idx_indexing_log_running
    ON ops_indexing_log(status)
    WHERE status = 'RUNNING';
CREATE INDEX IF NOT EXISTS idx_indexing_log_generation
    ON ops_indexing_log(generation_id);

-- ops_migration_log: tracks schema migration execution for crash recovery (Step R-3)
...
-- ops_retrieval_log: retrieval plan persistence (from retrieval_plan.md)
-- Column projections for efficient querying; plan_json is the authoritative record.
CREATE TABLE IF NOT EXISTS ops_retrieval_log (
...
CREATE INDEX IF NOT EXISTS idx_retrieval_log_workspace
    ON ops_retrieval_log(workspace_id, created_at_us DESC);
CREATE INDEX IF NOT EXISTS idx_retrieval_log_incomplete
    ON ops_retrieval_log(completed_at_us)
    WHERE completed_at_us IS NULL;
CREATE INDEX IF NOT EXISTS idx_retrieval_log_result
    ON ops_retrieval_log(result);

-- ops_freshness_log: audit log of freshness state transitions
...
CREATE INDEX IF NOT EXISTS idx_freshness_log_entity
...
-- ============================================================
-- SECTION 14: Record this Migration
-- ============================================================
